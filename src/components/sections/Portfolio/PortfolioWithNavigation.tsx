...
export default function PortfolioWithNavigation() {
  const [isStickyNavActive, setIsStickyNavActive] = useState(false);

  // Use intersection observer to hide portfolio nav when product grid is visible
  const handleProductGridVisibility = useCallback(() => {
    const productGrid = document.getElementById('product-grid');
    if (!productGrid) return;

    const observer = new IntersectionObserver(
      ([entry]) => {
        // Hide portfolio nav when product grid becomes visible
        setIsStickyNavActive(entry.isIntersecting);
      },
      {
        threshold: 0.1,
        rootMargin: '-100px 0px 0px 0px' // Account for nav height
      }
    );
  ...
  React.useEffect(() => {
    const cleanup = handleProductGridVisibility();
    return cleanup;
  }, [handleProductGridVisibility]);
  ...
  return (
    <>
            ...
            <div className="grid lg:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <p className="text-lg text-neutral-300 mb-6">
                  ...
                  and scales across small teams when you are ready.
                </p>
      ...
      {/* Product Grid Section with Sticky Navigation */}
      <motion.section 
        id="product-grid"
        initial="hidden"
        whileInView="visible"
