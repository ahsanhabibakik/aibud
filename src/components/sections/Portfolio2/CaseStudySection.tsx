        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {caseStudies.map((study, index) => (
            <div key={study.id} className="group">
              <div className="bg-neutral-900/50 border border-neutral-800/50 rounded-lg p-4 h-full hover:border-purple-500/20 transition-colors">
                <div className="flex flex-col h-full">
                  ...
                  <div className="mb-3">
                    <div className="bg-green-500/10 border border-green-500/20 rounded p-2">
                      <div className="text-center">
                        <div className="text-lg font-bold text-green-400">{study.outcome}</div>
                  ...
                  {/* Components Used */}
                  <div className="mb-4">
                    <div className="text-xs text-neutral-400 mb-1">Built with:</div>
                    <div className="flex flex-wrap gap-1">
                      {study.componentsUsed.slice(0, 2).map((component, idx) => (
                        <span key={idx} className="text-xs px-1.5 py-0.5 bg-purple-500/10 text-purple-400 rounded border border-purple-500/20">
                          {component}
                        </span>
                      ))}
                      {study.componentsUsed.length > 2 && (
                        <span className="text-xs px-1.5 py-0.5 bg-neutral-800/50 text-neutral-400 rounded border border-neutral-700/50">
                          +{study.componentsUsed.length - 2}
                        </span>
                      )}
                    </div>
