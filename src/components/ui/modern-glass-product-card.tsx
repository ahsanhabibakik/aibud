  ...
  return (
    ...
    >
      <motion.div
        ...
        className={cn(
          "relative h-full backdrop-blur-xl bg-black/40 border rounded-2xl md:rounded-3xl p-4 md:p-6 overflow-hidden transition-all duration-300",
          isHovered 
            ? `${categoryTheme.border} ${categoryTheme.glow} shadow-2xl bg-black/20` 
            : "border-white/10 shadow-lg shadow-black/25 hover:border-white/20"
        )}
      >
        ...
        <motion.div 
          ...
          style={{
            background: `linear-gradient(135deg, ${categoryTheme.gradient.replace('from-', '').replace('via-', '').replace('to-', '').split(' ').map((c: string) => c.replace('/', ',')).join(', ')})`
          }}
        />
